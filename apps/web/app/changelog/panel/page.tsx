...
export default function PanelChangelogPage() {
  ...
  async function toggleEntryHidden(entry: DbChangelogEntry) {
    ...
    try {
      const res = await fetch(`/api/changelogs/panel/entries/${entry.id}`, {
        method: "PATCH",
  ...
  const fetchEntries = useCallback(async () => {
    try {
      const res = await fetch("/api/changelogs/panel", { cache: "no-store" });
      if (res.ok) {
    ...
    <div className="min-h-screen bg-background p-8">
      ...
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {formEntryId ? "Editar entrada - Panel Web" : "Nueva entrada - Panel Web"}
            </DialogTitle>
          </DialogHeader>
          <NewChangelogForm
            key={formEntryId ?? "new"}
