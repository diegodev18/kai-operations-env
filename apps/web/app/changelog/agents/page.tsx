...
export default function AgentsChangelogPage() {
  const project = getProjectById("agents");
  ...
  async function toggleEntryHidden(entry: DbChangelogEntry) {
    ...
    try {
      const res = await fetch(`/api/changelogs/agents/entries/${entry.id}`, {
        method: "PATCH",
  ...
  const fetchEntries = useCallback(async () => {
    try {
      const res = await fetch("/api/changelogs/agents", { cache: "no-store" });
      if (res.ok) {
    ...
    <div className="min-h-screen bg-background p-8">
                  ...
                  <tr key={entry.id} className="hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <Link
                          href={`/changelog/agents/${entry.version}`}
      ...
      >
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {formEntryId ? "Editar entrada - kAI Agents" : "Nueva entrada - kAI Agents"}
            </DialogTitle>
          </DialogHeader>
          <NewChangelogForm
            key={formEntryId ?? "new"}
